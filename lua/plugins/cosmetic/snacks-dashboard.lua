...
return {
	...
	opts = {
		dashboard = {
			...
			preset = {
				keys = {
					{ icon = " ", key = "f", desc = "Find File " .. ("."):rep(61), action = ":lua Snacks.dashboard.pick('files')" },
					{ icon = " ", key = "g", desc = "Find Text " .. ("."):rep(61), action = ":lua Snacks.dashboard.pick('live_grep')" },
					{ icon = " ", key = "r", desc = "Recent Files " .. ("."):rep(58), action = ":lua Snacks.dashboard.pick('oldfiles')" },
					{ icon = " ", key = "s", desc = "Restore Session " .. ("."):rep(55), section = "session" },
					{ icon = " ", key = "n", desc = "New File " .. ("."):rep(62), action = ":lua vim.ui.input( { prompt = 'New File Name' }, function(input) vim.cmd('edit ' .. input .. ' | startinsert') end )" },
					{ icon = " ", key = "c", desc = "Config " .. ("."):rep(64), action = ":lua vim.cmd([[execute 'cd ' stdpath('config')]]) Snacks.dashboard.pick('files', {cwd = vim.fn.stdpath('config')})" },
					{ icon = "󰒲 ", key = "L", desc = "Lazy " .. ("."):rep(66), action = ":Lazy", enabled = package.loaded.lazy ~= nil },
					{ icon = " ", key = "q", desc = "Quit " .. ("."):rep(66), action = ":qa" },
				},
			},
			formats = {
